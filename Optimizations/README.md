...
- `algebraic_strength_reduction.c` / `algebraic_strength_reduction.h`
  - reduces arithmetic identities such as `x * 0`, `x * 1`, `x + 0`, `x - 0`, and `x / 1`
  - avoids adding shift operators or other parser-level syntax
...
```bash
gcc tests/test_optimizer_config.c optimizer.c constant_propagation.c algebraic_strength_reduction.c constant_folding.c dead_code_elimination.c ../SemanticRoutines/ast.c -o test_optimizer_config
